...
func NMEAchecksum(in string) string {
	// this assumes we get a complete string, with checksum at the end
	// it'd probably be sensible to eventually add checks for what is
	// actually received.
...
func handleData(c net.Conn) {
	...
	for {
		...
		// minimum expected line would be something like $GPGSA,*CC -
		// which is 10 characters. Though even that short one probably
		// should never happen.
		if len(line) < 10 {
		...
		calculated_checksum := NMEAchecksum(line)
		received_checksum := line[len(line)-2:]

		if calculated_checksum != received_checksum {
			fmt.Printf("Checksum mismatch, calculated %s but got %s\n",
